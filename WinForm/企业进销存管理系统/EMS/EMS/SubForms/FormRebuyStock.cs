...
namespace EMS.SubForms
{
    public partial class FormRebuyStock : Form
    {
        ...
        private void FormRebuyStock_Load(object sender, EventArgs e)
        {
            doperate.cboxBind("select BillCode from tb_BuyStock where BillCode like '%JH%'", "tb_BuyStock", "BillCode", cbbBillCode);
            ...
            ds = datacon.getds("select * from tb_BuyStock where BillCode like '%TH%'", "tb_BuyStock");//获取所有进货单信息
        ...
        private void CbbBillCode_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                DataSet ds = datacon.getds("select Unit from tb_BuyStock where BillCode='" + cbbBillCode.Text + "'", "tb_BuyStock");
                txtUnits.Text = ds.Tables[0].Rows[0][0].ToString();
                DataSet ds2 = datacon.getds("select GoodsCode,GoodsName,Standard,Num,Price,Money from tb_BuyStockDetail where BillCode='" + cbbBillCode.Text + "'", "tb_BuyStockDetail");
                DataTable dt = ds2.Tables["tb_BuyStockDetail"];
